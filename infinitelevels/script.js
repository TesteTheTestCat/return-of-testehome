...
function makeInnerHTMLForLevelTime(lv,xp){
  let colors = ["#ff0000","#ff8800","#ffff00","#88ff00","#00ff00","#00ff88","#00ffff","#0088ff","#0000ff","#8800ff","#ff00ff","#ff0088"]
  let j = ""
  ...
  <br>*${format(highRanks(lv,i))} boost from <div style="animation-name: rainbow; animation-duration:2s; animation-iteration-count:infinite; display: inline-block;">higher tiers</div><br><progress max="${levelRequire(lv[i],i)}" value="${xp[i]}"></progress><br>`
  }
  return j
}
function makeInnerHTMLForMetalevel(mlv,mxp){
  ...
  <br>/${format(1+(0.01*mlv))} cost decrease from <div style="animation-name: rainbow; animation-duration:2s; animation-iteration-count:infinite; display: inline-block;">metalevel</div><br><progress data-color="blue" max="${levelRequire(mlv,5,0)}" value="${mxp}"></progress><br><br>`
}
