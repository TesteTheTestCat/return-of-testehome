...
function makeInnerHTMLForLevelTime(lv,xp){
  ...
  <br>*${format(highRanks(lv,i))} boost from <div style="animation-name: rainbow; animation-duration:2s; animation-iteration-count:infinite;">higher tiers</div><br><progress max="${levelRequire(lv[i],i)}" value="${xp[i]}"></progress><br>`
  }
  return j
}
function makeInnerHTMLForMetalevel(mlv,mxp){
  ...
  <br>/${format(1+(0.01*mlv))} cost decrease from <div style="animation-name: rainbow; animation-duration:2s; animation-iteration-count:infinite;">metalevel</div><br><progress data-color="blue" max="${levelRequire(mlv,5,0)}" value="${mxp}"></progress><br><br>`
}
